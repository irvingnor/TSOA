...
import sistemaDistribuido.sistema.clienteServidor.modoUsuario.Framenombreserver;
...
public class MicroNucleoFrame extends Frame implements WindowListener,Escribano,ParMaquinaProceso{
	...
	private ProcesoFrame procesos[]=new ProcesoFrame[100];
	...
	class ManejadorBotones implements ActionListener{
		...
		public void actionPerformed(ActionEvent e){
			...
			if (com.equals("Cliente")){
				...
				Framenombreserver f = new Framenombreserver();
				f.setVisible(true);
				int idaguardar = Integer.valueOf(id);
				f.idarguardar = idaguardar;
				try {
					f.ip = InetAddress.getLocalHost().getHostAddress();
		...
		synchronized(procesos){
			for(int i=0;i<procesos.length;i++){
				if (procesos[i]!=null && procesos[i].equals(pf)){
					...
					Pausador.pausa(2000);
					pf.setVisible(false);
					int id = Integer.valueOf(pf.dameIdProceso());
					NameServer.deregistro(id);
					procesos[i]=null;
					break;
				}
			}
	...
	public static void main(String args[]){
		...
		NameServer na = new NameServer();
		na.setVisible(true);
	}

	@Override
	public String damenombre() {
	return destinatario.damenombre();
	}
}
